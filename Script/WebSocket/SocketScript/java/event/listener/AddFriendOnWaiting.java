...
public class AddFriendOnWaiting implements SocketEventListener.EventListener {
    ...
    public void run(SocketChannel channel, JSONObject jsonObject) {
        ...
        String friendId = jsonObject.getString(SocketEventListener.eKey.USER_ID1.toString());
        ...
        Map<String, Object> userData = MysqlManager.Instance().getUserByUserId(Integer.parseInt(userId));
        Map<String, Object> friendData = MysqlManager.Instance().getUserByUserId(Integer.parseInt(friendId));
        if(userData != null && friendData != null){
            ...
            //mysql 적용
            MysqlManager.Instance().addFriendOnWaiting(userWaiting, userFriends, Integer.parseInt(userId), friendWaiting, friendFriends, Integer.parseInt(friendId));

            JSONObject createDMChannelData = new JSONObject()
                    .put(SocketEventListener.eKey.TYPE.toString(), SocketEventListener.eType.CREATE_DM_CHANNEL)
